"use client";

import Link from "next/link";

const serviceLinks = [
    { id: "paid-media", name: "Paid Media" },
    { id: "seo", name: "SEO" },
    { id: "content", name: "Content Strategy" },
    { id: "email", name: "Email & Lifecycle" },
    { id: "analytics", name: "Analytics & Attribution" },
    { id: "creative", name: "Creative Production" },
    { id: "social-media", name: "Social Media" },
];

const industryLinks = [
    { slug: "ecommerce-retail", name: "E-commerce & Retail" },
    { slug: "saas-technology", name: "SaaS & Technology" },
    { slug: "healthcare-wellness", name: "Healthcare" },
    { slug: "finance-fintech", name: "Finance & Fintech" },
    { slug: "real-estate", name: "Real Estate" },
    { slug: "professional-services", name: "Professional Services" },
    { slug: "education-edtech", name: "Education & EdTech" },
    { slug: "hospitality-travel", name: "Hospitality & Travel" },
    { slug: "manufacturing-b2b", name: "Manufacturing & B2B" },
    { slug: "nonprofits-ngos", name: "Non-profits & NGOs" },
];

export const Footer = () => {
    return (
        <footer className="border-t border-white/10 bg-[#0a0a0a]">
            {/* Partner badges */}
            <div className="border-b border-white/5">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                    <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-12">
                        <div className="flex items-center gap-2 text-white/20">
                            <span className="material-symbols-outlined text-xl">verified</span>
                            <span className="font-mono text-[10px] uppercase tracking-wider">Google Partner</span>
                        </div>
                        <div className="flex items-center gap-2 text-white/20">
                            <span className="material-symbols-outlined text-xl">verified</span>
                            <span className="font-mono text-[10px] uppercase tracking-wider">Meta Business Partner</span>
                        </div>
                        <div className="flex items-center gap-2 text-white/20">
                            <span className="material-symbols-outlined text-xl">verified</span>
                            <span className="font-mono text-[10px] uppercase tracking-wider">HubSpot Solutions</span>
                        </div>
                        <div className="flex items-center gap-2 text-white/20">
                            <span className="material-symbols-outlined text-xl">verified</span>
                            <span className="font-mono text-[10px] uppercase tracking-wider">Shopify Plus</span>
                        </div>
                    </div>
                </div>
            </div>

            {/* Main footer content */}
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 lg:gap-12">
                    {/* Brand column */}
                    <div className="col-span-2 md:col-span-4 lg:col-span-1 mb-4 lg:mb-0">
                        <Link href="/" className="flex items-center gap-2 mb-4">
                            <span className="material-symbols-outlined text-white text-xl">view_in_ar</span>
                            <span className="text-sm font-bold tracking-widest uppercase">ScaleHauz</span>
                        </Link>
                        <p className="text-sm text-white/40 leading-relaxed mb-6 max-w-xs">
                            Full-stack growth marketing engineered to compound. We build the systems that scale your business.
                        </p>
                        <div className="flex items-center gap-4">
                            <a href="#" className="text-white/30 hover:text-white transition-colors" aria-label="LinkedIn">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                            </a>
                            <a href="#" className="text-white/30 hover:text-white transition-colors" aria-label="X / Twitter">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                            </a>
                            <a href="#" className="text-white/30 hover:text-white transition-colors" aria-label="Instagram">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                            </a>
                        </div>
                    </div>

                    {/* Services column */}
                    <div>
                        <h4 className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/50 mb-4">Services</h4>
                        <ul className="space-y-2.5">
                            {serviceLinks.map((service) => (
                                <li key={service.id}>
                                    <Link
                                        href={`/services/${service.id}`}
                                        className="text-sm text-white/35 hover:text-white transition-colors"
                                    >
                                        {service.name}
                                    </Link>
                                </li>
                            ))}
                        </ul>
                    </div>

                    {/* Industries column */}
                    <div>
                        <h4 className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/50 mb-4">Industries</h4>
                        <ul className="space-y-2.5">
                            {industryLinks.slice(0, 7).map((industry) => (
                                <li key={industry.slug}>
                                    <Link
                                        href={`/industries/${industry.slug}`}
                                        className="text-sm text-white/35 hover:text-white transition-colors"
                                    >
                                        {industry.name}
                                    </Link>
                                </li>
                            ))}
                            <li>
                                <span className="text-sm text-white/20">+ 3 more</span>
                            </li>
                        </ul>
                    </div>

                    {/* Company column */}
                    <div>
                        <h4 className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/50 mb-4">Company</h4>
                        <ul className="space-y-2.5">
                            <li><Link href="/work" className="text-sm text-white/35 hover:text-white transition-colors">Case Studies</Link></li>
                            <li><Link href="/pricing" className="text-sm text-white/35 hover:text-white transition-colors">Pricing</Link></li>
                            <li><Link href="/enterprise" className="text-sm text-white/35 hover:text-white transition-colors">Enterprise</Link></li>
                            <li><Link href="/#contact" className="text-sm text-white/35 hover:text-white transition-colors">Contact</Link></li>
                            <li><Link href="/#faq" className="text-sm text-white/35 hover:text-white transition-colors">FAQ</Link></li>
                            <li><Link href="/careers" className="text-sm text-white/35 hover:text-white transition-colors">Careers</Link></li>
                        </ul>
                    </div>
                </div>
            </div>

            {/* Bottom bar */}
            <div className="border-t border-white/5">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-xs font-mono text-white/20">
                        &copy; {new Date().getFullYear()} ScaleHauz. All rights reserved.
                    </div>
                    <div className="flex items-center gap-6 font-mono text-xs text-white/20">
                        <Link href="#" className="hover:text-white/50 transition-colors">Privacy Policy</Link>
                        <Link href="#" className="hover:text-white/50 transition-colors">Terms of Service</Link>
                        <Link href="#" className="hover:text-white/50 transition-colors">Cookies</Link>
                    </div>
                </div>
            </div>
        </footer>
    );
};
